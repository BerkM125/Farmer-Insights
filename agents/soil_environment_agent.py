...
def soil_advice(ph_stats: dict) -> str:
    ...
    if primary_ph < 5.5:
        advice_parts.append("🚨 SOIL IS HIGHLY ACIDIC - Apply lime immediately to raise pH to 6.0-6.5")
        ...
        advice_parts.append("🌱 Consider acid-tolerant crops like blueberries or potatoes")
        
    elif primary_ph > 8.0:
        advice_parts.append("🚨 SOIL IS HIGHLY ALKALINE - Apply sulfur or acidifying fertilizers")
        advice_parts.append("💡 Use elemental sulfur at 1-2 tons per acre")
        advice_parts.append("🌿 Consider alkaline-tolerant crops like asparagus or spinach")
    ...
    if p25_ph is not None and p75_ph is not None:
        ...
        if ph_range > 1.5:
            advice_parts.append("⚠️ HIGH pH VARIABILITY - Soil zones need different treatments")
            advice_parts.append("🗺️ Consider zone-specific liming/acidification")
            
        elif ph_range > 1.0:
            advice_parts.append("📊 MODERATE pH VARIABILITY - Monitor different field zones")
            advice_parts.append("🌾 Adjust fertilizer rates by zone")
    
    # Specific crop recommendations based on pH
    if primary_ph < 6.0:
        advice_parts.append("🌿 RECOMMENDED CROPS: Blueberries, potatoes, sweet potatoes, radishes")
    elif primary_ph > 7.5:
        advice_parts.append("🌿 RECOMMENDED CROPS: Asparagus, spinach, cabbage, broccoli")
    else:
        advice_parts.append("🌿 RECOMMENDED CROPS: Corn, soybeans, wheat, tomatoes, peppers")
...
def get_soil_ph(latitude: float, longitude: float) -> tuple[ee.Image, dict]:
    ...
    soil_pH = ee.Image('projects/soilgrids-isric/phh2o_mean').select(0).divide(10).clip(region)
    ...
    result = supabase_client.table("environmental_data").update(ph_data_block).eq("farm_id", "FARM01").execute()
...
def get_soil_nitrogen(latitude: float, longitude: float) -> list[float]:
    ...
    for day in range(days):
        ...
        nitrogen_level = (base_nitrogen * seasonal_factor + fertilizer_boost) * weather_factor - plant_uptake
