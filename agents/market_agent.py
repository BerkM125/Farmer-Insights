...
def is_base_crop_price(data_item, commodity, allow_subtypes=False):
    """
    Check if the data item is a base crop price.

    Args:
        data_item: The data item string from CSV
        commodity: The commodity name
        allow_subtypes: If True, allows only "GRAIN" subtype patterns like "CORN, GRAIN - PRICE RECEIVED..."

    Examples:
        "BARLEY - PRICE RECEIVED, MEASURED IN $ / BU" -> True (standard)
        "BARLEY, FEED - PRICE RECEIVED, MEASURED IN $ / BU" -> False (both modes)
        "CORN, GRAIN - PRICE RECEIVED, MEASURED IN $ / BU" -> False (standard), True (with allow_subtypes)
    """
    if not allow_subtypes:
        # Standard pattern: COMMODITY - PRICE RECEIVED, MEASURED IN ...
        ...
        # Specific pattern: COMMODITY, GRAIN - PRICE RECEIVED, MEASURED IN ...
        # Only matches GRAIN subtype
        pattern = (
            rf"^{re.escape(commodity.upper())}, GRAIN - PRICE RECEIVED, MEASURED IN .+$"
        )
        return bool(re.match(pattern, data_item))
...
def load_crop_price_data(crop_type):
    ...
    First tries to match standard pattern (e.g., "BARLEY - PRICE RECEIVED...").
    If no results found, tries broader pattern (e.g., "CORN, GRAIN - PRICE RECEIVED...").
    """
    csv_path = os.path.join(os.path.dirname(__file__), "data", "prices.csv")

    # Try standard pattern first
    price_records = _load_price_data_with_pattern(
        csv_path, crop_type, allow_subtypes=False
    )

    # If no results, try broader pattern that includes subtypes
    if not price_records:
        price_records = _load_price_data_with_pattern(
            csv_path, crop_type, allow_subtypes=True
        )

    return price_records


def _load_price_data_with_pattern(csv_path, crop_type, allow_subtypes):
    """
    Helper function to load price data with a specific pattern.
    """
    price_records = []
    ...
    with open(csv_path, "r") as file:
        ...
        for row in reader:
            ...
            # Check if this matches the desired pattern
            if not is_base_crop_price(data_item, commodity, allow_subtypes):
                continue
...
async def handle_market_request(ctx: Context, sender: str, msg: MarketRequest):
    ...
    try:
        ...
        if not price_records:
            ctx.logger.warning(
                f"No price data found for crop '{msg.crop_type}' in the last 12 months"
            )
        ...
        ctx.logger.info(
            f"📊 Sending response: {len(price_record_list)} records, latest: ${latest_record['price']} on {latest_record['date']}"
        )
        await ctx.send(sender, response)
